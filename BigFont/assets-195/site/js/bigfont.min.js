...
﻿(function(){'use strict';var BASE_URL,BOOTSTRAP_COLLAPSE_DURATION;BASE_URL=window.location.protocol+'//'+window.location.hostname+':'+window.location.port;BOOTSTRAP_COLLAPSE_DURATION=600;function setupTheAnchorElementNonLinkBehavior(){$("a.non-link").click(function(e){e.preventDefault();e.stopPropagation();return false;});}
function expandTheCollapsingContactFormBySectionHash(hash){$('section'+hash+' '+'.collapse').collapse('show');}
function repositionTheCollapsingContactForm(form){var offsetTop,newPosition;offsetTop=form.offset().top;if($('button.btn-navbar').is(':visible')){newPosition=offsetTop-5-60;}else{newPosition=offsetTop-50-60;}
$('html,body').scrollTop(newPosition);}
function setupTheCollapsingContactForm(){var div,button,strong,span,forms,form,isValid,validator,jqxhr,scrollTopShow,offsetTopShow,scrollTopShown,offsetTopShown;forms=$('form.contact-form');forms.children('.collapse').bind('show',function(){scrollTopShow=$(window).scrollTop();offsetTopShow=$(this).offset().top;$(".alert").alert();forms.find('.alert').alert('close');$('.collapse.in').height(0).removeClass('in');offsetTopShown=$(this).offset().top;scrollTopShown=scrollTopShow-(offsetTopShow-offsetTopShown);$(window).scrollTop(scrollTopShown);});forms.children('.collapse').bind('shown',function(){$(this).find('[name=fromEmail]').focus();});forms.children('.collapse').bind('hidden',function(){form=$(this).parents('form');form.validate().resetForm();form[0].reset();});$.each(forms,function(i){form=$(forms[i]);validator=form.validate({rules:{fromEmail:{required:true,email:true},fromName:"required",subject:"required",body:"required"},messages:{fromEmail:{required:'Please provide your email.',email:'Please provide a valid email.'},fromName:"Please provide your name.",subject:"Please provide a subject.",body:"Please type a message."}});});forms.find('button[type=submit]').click(function(){form=$(this).parents('form');isValid=form.valid();if(isValid){div=$('<div/>',{'class':'alert alert-success'});button=$('<button/>',{type:'button','class':'close','data-dismiss':'alert',html:'x'});strong=$('<strong/>',{html:'Thank you.&nbsp'});span=$('<span/>',{html:'We will reply shortly.'});div.append(button);div.append(strong);div.append(span);form.append(div);repositionTheCollapsingContactForm(form);}
...
function setupNavigationClickHandler(){var collapsible,href,doDefault;$('nav#sub-nav a').click(function(e){collapsible=$('.in.collapse');if(collapsible.length>0){href=$(this).attr('href');collapsible.collapse('hide');window.setTimeout(function(){window.location.hash=href;},BOOTSTRAP_COLLAPSE_DURATION);e.preventDefault();doDefault=false;}});return doDefault;}
